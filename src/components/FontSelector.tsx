      ...
      <CardHeader className="bg-gradient-to-r from-slate-50 to-blue-50 border-b">
        <CardTitle className="font-display flex items-center gap-3">
          <div className="relative">
            ...
            <Palette className="h-3 w-3 text-indigo-500 absolute -bottom-1 -right-1" />
          </div>
          Font Selection
          {isPremium && (
      ...
      <CardContent className="p-6 space-y-6">
        <div>
          ...
          <Select value={value} onValueChange={onChange}>
            <SelectTrigger className="bg-gradient-to-r from-slate-50/50 to-blue-50/50 border-slate-200 focus:border-blue-400 h-12 text-lg rounded-xl">
              <SelectValue placeholder="Choose your perfect font..." />
            </SelectTrigger>
            <SelectContent className="bg-white border-slate-200 shadow-2xl max-h-80 rounded-xl">
              {cursiveFonts.length > 0 && (
                <>
                  <div className="px-3 py-2 text-sm font-bold text-blue-700 bg-blue-50 rounded-t-lg">
                    ...
                    >
                      <div className="flex items-center justify-between w-full">
                        ...
                        {isPremium && !FONT_COLLECTIONS.free.cursive.some(f => f.id === font.id) && (
                          <Crown className="h-4 w-4 text-blue-600 ml-2" />
                        )}
                      </div>
                    </SelectItem>
                  ))}
              ...
              {printedFonts.length > 0 && (
                <>
                  <div className="px-3 py-2 text-sm font-bold text-indigo-700 bg-indigo-50 mt-2">
                  ...
                  {printedFonts.map((font) => (
                    <SelectItem 
                      key={font.id} 
                      value={font.id} 
                      className="hover:bg-indigo-50 py-3 px-4 cursor-pointer"
                    >
                      <div className="flex items-center justify-between w-full">
                        <span 
                          style={{ fontFamily: font.family }} 
          ...
          {!isPremium && (
            <div className="mt-4 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-xl">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="font-semibold text-blue-800">Unlock Premium Fonts</div>
